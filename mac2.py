    ...
    if net_connect is not None:
        ...
        for item in arp_list:
            ...
            if ip_result is not None and mac_result is not None:
                ...
                device_info = {'ip': ip_result, 
                               'hostname': hostname['hostnames'], 
                               'mac': mac_result, 
                               'status': hostname['status']}

                mac_list.append(device_info)
            else: 
                continue

            club_results[club_result] = mac_list
        print(club_results)

    print(club_results)
    print(club_results.keys())
    output = open('inventory3.json', 'a+')
    output.write(json.dumps(club_results))
    output.close()
...
def main():
    ...
    for ip in ip_list:
        club_results = getMacAddress(ip)
        final_list.append(club_results)   
        output = open('inventory10.json', 'a+')
        output.write(json.dumps(final_list))
        output.close()
