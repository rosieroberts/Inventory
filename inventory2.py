...
class clubResult(AuditResults):
    def __init__(self, item):
        AuditResults.__init__(self)
        self.ip = item 
        ...
        for _ in range(1):
            for attempt in range(2):
                ...
                try:
                    ...
                    nmap_args = 'p22'
                    scanner = PortScanner()
                    scanner.scan(hosts=self.ip, arguments=nmap_args)

                    for self.ip in scanner.all_hosts():
                        if scanner[self.ip].has_tcp(22):
                            if scanner[self.ip]['tcp'][22]['state'] == 'closed':
                                print('port 22 is showing closed for ' + (self.ip))
                                not_connected.append(self.ip)
                                return None
                            else:
                                print('Port 22 is open ')
                                break
                        else:
                            print('port 22 is closed for ' + (self.ip))
                            continue
                    print('Connecting... attempt', attempt + 1)
                    if attempt == 0:
                        print('Error, Trying to connect to {} again '.format(self.ip))
            # exhausted all tries to connect, return None and exit
            print('Connection to {} is not possible: '.format(self.ip))
            not_connected.append(self.ip)
            return None
    ...
    def diff(results):
        ...
        if len(diff2) != 0:
            for diff2_item in diff2:
                ...
                # returns None if not found
                results_item_id = next((item for item in results if item['ID'] ==
                                        diff2_item['ID']), None)
                # returns dict item if mac address is found in results
                results_item_mac = next((item for item in results if
                                        item['Mac Address'] ==
                                        diff2_item['Mac Address']), None)
                # if diff2_item Id and Mac cannot be found in results
        ...
        if len(all_diff) != 0:
            for item in baseline_review:
                print('review')
                print(item)
            for item in baseline_update:
                print('update')
                print(item)
            for item in baseline_add:
                print('add')
                print(item)
            for item in baseline_remove:
                print('remove')
                print(item)
...
for item in get_ip_list():
    obj_ip = clubResult(item)
    obj_host = obj_ip.get_site_router()
    con_obj = obj_ip.connect(obj_host)
